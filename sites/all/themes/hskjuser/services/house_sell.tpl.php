...
<div class="rich_media">
    <div class="rich_media_inner">
      <div style="font-size:17px;padding-bottom:10px;padding-top:10px"><strong>价格：<span style="color:#ff4444;">
	  <?php
...
<?php		
	  ...
	  //房屋朝向
	   if(is_array($node->field_face)){ 
	    ...
	    //产权年代
	   if(is_array($node->field_cqnd)){ 
	    foreach($node->field_cqnd as $key=>$var){
          if(is_array($var)){
	         foreach($var as $key1=>$var1){
					foreach($var1 as $k=>$v){
				    if($k=="tid")
					$cqnd = get_taxonomy_namy_by_tid($v);
					}
			 }
		  }
	    }
	    ...
	    //产权类型
	   if(is_array($node->field_rights)){ 
	    foreach($node->field_rights as $key=>$var){
          if(is_array($var)){
	         foreach($var as $key1=>$var1){
					foreach($var1 as $k=>$v){
				    if($k=="tid")
					$rights = get_taxonomy_namy_by_tid($v);
					}
			 }
		  }
	    }
	  ...
	  //建筑年代
	  if(is_array($node->field_years)){ 
	    foreach($node->field_years as $key=>$var){
          if(is_array($var)){
	         foreach($var as $key1=>$var1){
					foreach($var1 as $k=>$v){
				    if($k=="value")
...
?>
	  ...
	  <table width="100%" style="line-height:1.6em;">
	 ...
	 <tr>
	  ...
	  <tr>
	    <td width="50%" align="left">产权：<?php print $cqnd; ?><?php print $rights; ?></td> <td width="50%"  align="left">楼层：<?php print $floor; ?>/<?php print $floors; ?></td>
	  </tr>  
	   <tr>
	    <td width="50%" align="left">年代：<?php print $years; ?></td> <td width="50%"  align="left">朝向：<?php print $face; ?></td>
	  </tr>   
